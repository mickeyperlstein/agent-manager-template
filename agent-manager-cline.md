# Agent Manager — Cline Rules

This file contains the Kanban workflow rules for Cline agents.
...
4. **Confirm your column** — if your work is not in one of those columns, stop and tell the human
...
2. **Never move a feature between columns** — one exception: the HLD agent MUST move a completed HLD to `HLD-Review`. All other column moves are human commits only.
3. **Never suggest fast-tracking.** If you find yourself about to say "we could skip" — stop. That is a process violation.
4. **Never implement work that belongs to a future column.** Stay in your lane.
